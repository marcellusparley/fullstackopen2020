...
describe('most blogs', () => {
  ...
  test('of bigger list is Rob M with 3', () => {
    const expected = { author: 'Robert C. Martin', blogs: 3 }
    expect(listHelper.mostBlogs(biggerList)).toEqual(expected)
  })
})
...
describe('most likes', () => {
  ...
  test('when list has one blog, equals that', () => {
    const expected = { author: listWithOneBlog[0].author,
      likes: listWithOneBlog[0].likes }
    expect(listHelper.mostLikes(listWithOneBlog)).toEqual(expected)
  })

  test('of bigger list is Edsger D with 17', () => {
    const expected = { author: 'Edsger W. Dijkstra', likes: 17 }
    expect(listHelper.mostLikes(biggerList)).toEqual(expected)
  })
})